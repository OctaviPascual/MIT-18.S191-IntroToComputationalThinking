...
ex_1_1 = md"""
We will show that for all a such that 0 < a < x, sqrt(x) is between x/a and a.

sqrt(x) is between x/a and a can be written as a < sqrt(x) < x/a OR x/a <= sqrt(x) <= a.

We can divide the proof in two cases: a < sqrt(x) and a >= sqrt(x).

- Case 1: a < sqrt(x)
a < sqrt(x) 

=> a*sqrt(x) < x (multiply by sqrt(x) > 0)

=> sqrt(x) < x/a (divide by a > 0)
...
function area_sierpinski(n)
	return (3/4)^n # area_sierpinksi(n) = 3/4 * area_sierpinski(n-1)
end
