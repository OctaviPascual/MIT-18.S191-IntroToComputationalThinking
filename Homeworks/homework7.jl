...
md"""
👉 Convert `S` to an integer using the `Integer` function. What value does it have? What values do `I` and `R` have?
"""

# ╔═╡ f2792ff5-b0b6-4fcd-94aa-0b6ef048f6ab
Integer(S)

# ╔═╡ 2e12c204-5b93-40b7-897a-f46f7227cc50
Integer(I)

# ╔═╡ c370b1e2-5623-4342-b816-1bab3633f883
Integer(R)
...
# ╔═╡ 60ca3bc7-f2b8-4d94-9bb1-1db62b03a234
begin
...
# ╔═╡ 93678cb2-6d1f-464e-84b8-f779789bc76b
...
md"""
#### Exercise 1.5
👉 Write a function `interact!` that takes an affected `agent` of type `Agent`, a `source` of type `Agent` and an `infection` of type `InfectionRecovery`.  It implements a single (one-sided) interaction between two agents: 
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	R_mean = []
	for i=1:T
		push!(S_mean, sum([sim.S[i] for sim in simulations]) / N)
		push!(I_mean, sum([sim.I[i] for sim in simulations]) / N)
		push!(R_mean, sum([sim.R[i] for sim in simulations]) / N)
	end
	plot!(p, 1:T, S_mean, alpha=.5, label="S_mean")
	plot!(p, 1:T, I_mean, alpha=.5, label="I_mean")
	plot!(p, 1:T, R_mean, alpha=.5, label="R_mean")
...
@bind p_infection Slider(0 : 0.01 : 1, show_value=true)

# ╔═╡ da76e683-cfc4-46b3-a09c-bcffea5024c1
@bind p_recovery Slider(0 : 0.01 : 1, show_value=true)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	for i=1:T
		push!(S_mean, sum([sim.S[i] for sim in simulations]) / N)
		push!(S_σ, std([sim.S[i] for sim in simulations]))
	
		push!(I_mean, sum([sim.I[i] for sim in simulations]) / N)
		push!(I_σ, std([sim.I[i] for sim in simulations]))
		
		push!(R_mean, sum([sim.R[i] for sim in simulations]) / N)
		push!(R_σ, std([sim.R[i] for sim in simulations]))
	end
	plot!(p, 1:T, S_mean, alpha=.1, label="S_mean", yerr=S_σ)
	plot!(p, 1:T, I_mean, alpha=.1, label="I_mean", yerr=I_σ)
	plot!(p, 1:T, R_mean, alpha=.1, label="R_mean", yerr=R_σ)
...
# ╔═╡ 63be96be-d2ef-4334-9d7b-9f6871e85624
...
# ╔═╡ 8b72c26c-537f-4182-84b0-c66cdce42bc4
...
# ╔═╡ 180e292b-f11f-459b-9322-af015dbf49cd
...
md"""
We can characterise the magnitude of the epidemic by measuring:
- percentage of infected people at the end of the simulation
- percentage of recovered people at the end of the simulation
- mean of number of infected at the end of the simulation
"""
...
let
	...
	mean = []
	for i=1:1000
		push!(mean, sum([sim.I[i] for sim in simulations]) / length(simulations))
	end
	plot!(p, 1:1000, mean, alpha=.5, label="mean", lw=3)
...
# ╔═╡ 147e8f8e-2091-4881-95b8-f3075c66cead
sir_mean_error_plot(simulations)

# ╔═╡ 5984ff0e-3332-41cb-adbc-cc236c479025
begin
	sims = repeat_simulations(100, 1000, InfectionRecovery(p_infection, p_recovery), 20)
	sir_mean_plot(sims)
end


# ╔═╡ a6a00964-f896-4a74-b841-8fde7f8c509f
function run_agents_simulation()
	N = 10_000
	T = 1000
	agents = generate_agents(N)
...
# ╔═╡ b7b9e1ee-3e44-4ecb-9a0d-80c1f6014dbd
agents_simulation = run_agents_simulation()

# ╔═╡ 26e2978e-0435-11eb-0d61-25f552d2771e
let
	xs, ps = probability_distribution([a.num_infected for a in agents_simulation])
	
	bar(xs, ps, alpha=0.5, leg=false)
end

# ╔═╡ 150b0ce3-c782-4d40-a7ba-71b929c07150
count(x -> x.status == I, agents_simulation) / length(agents_simulation) * 100

# ╔═╡ 387b5b48-062b-4bb3-9d74-059386129272
count(x -> x.status == R, agents_simulation) / length(agents_simulation) * 100

# ╔═╡ 4f2a0694-a9ee-4f05-a734-18feb414471d
sum([x.num_infected for x in agents_simulation]) / length(agents_simulation)
...
# ╔═╡ 25db027e-160c-4b81-b011-4873eaac861a
reinfection_simulations = repeat_simulations(100, 1000, Reinfection(0.02, 0.002), 20)

# ╔═╡ 1ac4b33a-0435-11eb-36f8-8f3f81ae7844
let
	p = plot()
	
	for sim in reinfection_simulations
		plot!(p, 1:1000, sim.I, alpha=.5, label=nothing)
	end

	mean = []
	for i=1:1000
		push!(mean, sum([sim.I[i] for sim in reinfection_simulations]) / length(reinfection_simulations))
	end
	plot!(p, 1:1000, mean, alpha=.5, label="mean", lw=3)
...
sir_mean_plot(reinfection_simulations)

# ╔═╡ f18d12c7-748b-4572-a108-1f91719e9183
md"""
We see that the number of recoveries stays at 0 because now agents can't recover.

The number of infections stabilizes around a value, as well as the number of susceptible. Hence, the system seems to converge to an equilibrium, whereas in the previous model we saw that the number of infections decreased by the end of the simulation.
"""
