...
function trajectory(c::Coordinate, n::Int, L::Number)
	op = (c1, c2) -> collide_boundary(c1 + c2, L)
	return accumulate(op, rand(possible_moves, n), init=c)
end
...
let
	...
	origin = Coordinate(0, 0)
	[plot_trajectory!(p, trajectory(origin, 1000)) for _ in 1:10]

	p
end

# ╔═╡ 0e655106-76c0-44fc-a726-02bcddf6caf8
md"""
We can check that if we use trajectory(origin, 100, 10) we never exit the 20x20 square centered around the origin:
"""

# ╔═╡ cab52c1a-1102-438a-93e2-e9297c61683f
let
	p = plot(ratio=1)

	origin = Coordinate(0, 0)
	[plot_trajectory!(p, trajectory(origin, 500, 10)) for _ in 1:100]
...
# ╔═╡ 2f97f9c7-4336-4140-9215-17cc2d698f0d
rand([4,5,6])
...
# ╔═╡ 5971929a-31d6-4ba9-8ce8-df9e7a40c907
...
causes_outbreak = CollisionInfectionRecovery(0.6, 0.00001)

# ╔═╡ 269955e4-0a46-11eb-02cc-1946dc918bfa
does_not_cause_outbreak = CollisionInfectionRecovery(0.8, 0.0001)
...
We need different parameters because the way agents interact is different: they only can get infected if they are in the same position.
"""
...
# ╔═╡ 81ffabb0-9b7c-43e7-8698-6b6ba9ea63fd
LinRange(0.1, 0.5, 10)
...
# ╔═╡ 76ab9fb9-ba5d-426e-868f-871310381a2d
...
function visualize(agents::Vector, L)
	...
	scatter!(p, make_tuple.(position.(agents)), c=color.(agents), label=nothing)
	return p
end
...
# ╔═╡ b6934422-72f0-4234-be2d-9dda75e9adc1
function is_infected(agent::AbstractAgent)
	return agent.status == I
end

# ╔═╡ c05ac38a-13d8-4bbe-b2cc-f4742203af58
...
function interact!(agent::Agent, source::Agent, infection::AbstractInfection)
	...
	if position(agent) == position(source) && is_susceptible(agent) && is_infected(source)
...
# ╔═╡ aae9c3f0-f19b-4a11-a850-8d32f9bd949e
function set_num_infected!(agent::SocialAgent, new_num_infected::Int64)
	agent.num_infected = new_num_infected
end

# ╔═╡ 7004d786-8a41-4c91-a9b1-6f5142a682aa
...
function initialize_social(N, L)
	...
	agents = [SocialAgent(Coordinate(rand(-L:L), rand(-L:L)), S, 0, rand(social_scores)) for _ in 1:N]
	rand(agents).status = I
	return agents
end
...
# ╔═╡ 99592d8e-74ff-4de0-8d2e-9984777dddfe
function is_interaction_risky(sa1::SocialAgent, sa2::SocialAgent)
	return bernoulli(sa1.social_score + sa2.social_score)
end

# ╔═╡ 465e918a-0a69-11eb-1b59-01150b4b0f36
function interact!(agent::SocialAgent, source::SocialAgent, infection::AbstractInfection)
	if is_infected(agent)
		if bernoulli(infection.p_recovery)
			agent.status = R
		end
	...
	if position(agent) == position(source) && is_susceptible(agent) && is_infected(source) && is_interaction_risky(agent, source)
		if bernoulli(infection.p_infection)
			agent.status = I
...
# ╔═╡ 0583b8c3-7b0f-422a-b3a9-ec55dcdd5e5d
let
	agent = Agent(Coordinate(0, 0), S)
	source = Agent(Coordinate(0, 0), I)
	infection = CollisionInfectionRecovery(0.5, 1)
...
# ╔═╡ af681ee0-cde3-4d75-b541-1f42e90dcf94
function sweep!(agents::Vector, L::Number, infection::AbstractInfection)
	[step!(agents, L, infection) for _ in 1:length(agents)]
end
...
let
	...
	[sweep!(agents, L, pandemic) for _ in 1:k_sweeps]
...
# ╔═╡ 756d9de1-b144-44a8-ab42-51887a77f9e6
function simulation(N::Integer, L::Integer, k_sweeps::Integer, infection::AbstractInfection)
	agents = initialize(N, L)
	...
	for _=1:k_sweeps
		...
		push!(S_counts, count(x -> x.status == S, agents))
		push!(I_counts, count(x -> x.status == I, agents))
		push!(R_counts, count(x -> x.status == R, agents))
	end
...
# ╔═╡ c45f20a7-4cd4-4a35-932c-6e63625f9e00
let
	N = 100
	L = 20
	
	sim = simulation(N, L, k_sweep_max, causes_outbreak)
...
let
	...
	sim = simulation(N, L, k_sweep_max, does_not_cause_outbreak)
...
let	
	...
	for _ in 1:50
		sim = simulation(N, L, k_sweeps, pandemic)

		plot!(p, 1:k_sweeps, sim.S, alpha=.5, c=:blue, label=nothing)
		plot!(p, 1:k_sweeps, sim.I, alpha=.5, c=:red, label=nothing)
		plot!(p, 1:k_sweeps, sim.R, alpha=.5, c=:green, label=nothing)
	end
...
let
	...
	@gif for t in 1:Tmax
		...
		plot!(right, 1:t, S_counts, color=color(S), label="S")
		plot!(right, 1:t, I_counts, color=color(I), label="I")
		plot!(right, 1:t, R_counts, color=color(R), label="R")
...
let
	...
	@gif for t in 1:Tmax
		...
		push!(S_counts, count(x -> x.status == S, social_agents))
		push!(I_counts, count(x -> x.status == I, social_agents))
		push!(R_counts, count(x -> x.status == R, social_agents))
...
function social_simulation!(agents::Vector, N::Integer, L::Integer, k_sweeps::Integer, infection::AbstractInfection)
	S_counts,I_counts, R_counts = [], [], []
...
# ╔═╡ 2600191f-7c7e-417a-84aa-4baeabf9c1dd
let
	N = 100
	L = 20

	agents = initialize_social(N, L)
	social_simulation!(agents, N, L, k_sweeps, pandemic)
	scatter(num_infected.(agents), social_score.(agents),
		label=nothing, xaxis="num_infected", yaxis="social_score")
end

# ╔═╡ 247345e2-0005-4327-8168-67735ced12bc
md"""
We see there is a correlation between the social score and the number of infected. In other words, the agents with higher social score tend to infect more people.
"""
...
let
	...
	agents = initialize_social(N, L)
	sim1 = social_simulation!(agents, N, L, lockdown, pandemic)

	map(x -> x.social_score *= 0.25, agents)

	sim2 = social_simulation!(agents, N, L, lockdown, pandemic)

	result = plot(1:2*lockdown, [sim1.S; sim2.S], label="Susceptible")
	plot!(result, 1:2*lockdown, [sim1.I; sim2.I], label="Infectious")
	plot!(result, 1:2*lockdown, [sim1.R; sim2.R], label="Recovered")
	vline!(result, [lockdown], label="Lockdown")
end
