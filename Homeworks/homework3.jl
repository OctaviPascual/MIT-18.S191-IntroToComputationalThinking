...
function clean(text)
	unaccent_text = unaccent(text)
	lowercase_text = map(lowercase, unaccent_text)
	return filter(isinalphabet, lowercase_text)
end
...
unused_letters = map(i -> alphabet[i], findall( ==(0), sample_freqs))
...
# ╔═╡ bb9a7af8-4a9e-48de-ab1b-0de4e0a53662
double_letter_freq = [(alphabet[i], sample_freq_matrix[i, i]) for i in 1:length(alphabet)]

# ╔═╡ 65c92cac-f930-11ea-20b1-6b8f45b3f262
double_letters = map(first, filter(t -> t[2] > 0, double_letter_freq))
...
# ╔═╡ c8316d5c-d6b7-41eb-be67-3c776350f58e
w_star_freq = [(alphabet[i], sample_freq_matrix[index_of_letter('w'), i]) for i in 1:length(alphabet)]

# ╔═╡ 7898b76a-f930-11ea-2b7e-8126ec2b8ffd
most_likely_to_follow_w = first(reduce((x,y) -> y[2] > x[2] ? y : x, w_star_freq))
...
# ╔═╡ d565d667-2d04-4b00-a983-5e0a1ba9b204
star_w_freq = [(alphabet[i], sample_freq_matrix[i, index_of_letter('w')]) for i in 1:length(alphabet)]

# ╔═╡ bc401bee-f931-11ea-09cc-c5efe2f11194
most_likely_to_precede_w = first(reduce((x,y) -> y[2] > x[2] ? y : x, star_w_freq))
...
row_sums = [sum(sample_freq_matrix[i, :]) for i in 1:length(alphabet)]

# ╔═╡ 4a0314a6-7dc0-4ee9-842b-3f9bd4d61fb1
col_sums = [sum(sample_freq_matrix[:, i]) for i in 1:length(alphabet)]

# ╔═╡ 33a3f001-83e5-4e36-8535-ebe9d645d1a9
sum(col_sums) ≈ sum(row_sums) ≈ 1

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""
The sum of each row i is the frequency of transitions whose first letter is i.

The sum of each column i is the frequency of transitions whose second letter is i.

The sum of the matrix is the sum of all transitions frequency, hence 1.
"""
