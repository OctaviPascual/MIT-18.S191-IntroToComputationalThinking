...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 1:m-1
		south_west = energies[i+1, clamp(current_pixel-1, 1, n)]
		south      = energies[i+1, current_pixel]
		south_east = energies[i+1, clamp(current_pixel+1, 1, n)]
		...
		_, index = findmax(identity, [south_west, south, south_east])
		current_pixel += [-1, 0, 1][index]
		push!(seam, current_pixel)
	end
...
function least_energy(energies, i, j)
	...
	## induction
	south_west, _ = least_energy(energies, i+1, j-1)
	south, _      = least_energy(energies, i+1, j)
	south_east, _ = least_energy(energies, i+1, j+1)

	min_energy, index = findmin(identity, [south_west, south, south_east])
	return (min_energy + energies[i, j], j + [-1, 0, 1][index])
end
...
function recursive_seam(energies, starting_pixel)
	...
	for i in 1:m-1
		_, current_pixel = least_energy(energies, i, current_pixel)
		push!(seam, current_pixel)
	end
...
This algorithm does an exhaustive search of all possible paths as it calls least_energy function for each row of the image. The contract of least_energy is that it returns the lowest possible energy sum at the current pixel and the the column to jump to in next row.

At every row, we have 3 possible jumps to make. So the number of possible seams for an image is O(n * 3^m).
"""
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	## induction
	south_west, _ = memoized_least_energy(energies, i+1, j-1, memory)
	south, _      = memoized_least_energy(energies, i+1, j, memory)
	south_east, _ = memoized_least_energy(energies, i+1, j+1, memory)

	min_energy, index = findmin(identity, [south_west, south, south_east])
	memory[(i, j)] = (min_energy + energies[i, j], j + [-1, 0, 1][index])
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	for i in 1:m-1
		_, current_pixel = memoized_least_energy(energies, i, current_pixel, memory)
		push!(seam, current_pixel)
	end
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
	...
	## induction
	south_west, _ = matrix_memoized_least_energy(energies, i+1, j-1, memory)
	south, _      = matrix_memoized_least_energy(energies, i+1, j, memory)
	south_east, _ = matrix_memoized_least_energy(energies, i+1, j+1, memory)
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for i in 1:m-1
		_, current_pixel = matrix_memoized_least_energy(energies, i, current_pixel, memory)
		push!(seam, current_pixel)
	end
...
function least_energy_matrix(energies)
	...
	for i in m-1:-1:1
		for j in 1:n
			south_west = energies[i+1, clamp(j-1, 1, n)]
			south      = energies[i+1, j]
			south_east = energies[i+1, clamp(j+1, 1, n)]
			...
			result[i, j] = energies[i, j] + max(south_west, south, south_east)
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	return greedy_seam(least_energies, starting_pixel)
