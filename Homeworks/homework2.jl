...
@bind l_box Slider(1:30, show_value=true, default=1)
...
function extend(M::AbstractMatrix, i, j)
	...
	return M[clamp(i, 1, num_rows), clamp(j, 1, num_cols)]
end
...
function box_blur(v::AbstractArray, l)
	...
	for i in range(1, length(v))
		window = [extend(v, i) for i in i-l:i+l]
...
# ╔═╡ 95b710b5-ac02-46ac-b1a8-26df46d30554
typeof(box_blur)
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in range(1, num_rows)
		for j in range(1, num_cols)
			window1 = [extend(M, i, j) for i in i-k′:i+k′, j in j-l′:j+l′]
			window2 = [K[i, j] for i in 1:k, j in 1:l]
			R[i, j] = sum(window1 .* window2)
		end
	end
...
function gaussian_kernel_1D(n; σ = 1)
	result = [gauss(i, σ) for i in 1:(2*n + 1)]
...
# ╔═╡ 784296b5-e561-4260-a25c-6cbdf6e3e5fd
function gaussian_kernel_2D(l, σ)
	l′ = 2*l + 1
	result = [gauss(i, j; σ) for i in 1:l′ for j in 1:l′]
	result = reshape(result, l′, l′)
	return result ./ sum(result)
end

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image; σ=3, l=5)
	return convolve(image, gaussian_kernel_2D(l, σ))
end
...
When `face_σ` is set to a low number, `face_l` has the effect of zooming in the image towards the center. When `face_l` is set to a low number, `face_σ` has the effect of blurring the image.
...
# ╔═╡ bbd73443-d9e4-47f2-b5ff-90ecd79fa500
function sobel_kernel_2D()
...
function with_sobel_edge_detect(image)
	return convolve(image, sobel_kernel_2D())
end
